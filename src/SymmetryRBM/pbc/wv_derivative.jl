...
function transl_inv_tool(nspins::Int64, alpha::Int64,L_x::Int64,L_y::Int64)
  ...
  #Matrix storing the filter index matrix for translation invariance
...
# function Var_derivatives(state::Array{Int64,1},ENWfDer::Array{T,2},Lt::Array{T,1},FilterInd::Array{Int64,2},nspins::Int64,alpha::Int64, ind::Int64) where {T <: Number}
...
#Calculate the derivative of the wavefunction respect to the variational parameters for a given state
function Var_derivatives!(state::Array{Int64,1}, derivative::Array{T,1}, Lt::Array{T,1}, FilterInd::Array{Int64,2}, nspins::Int64, alpha::Int64) where {T <: Number}
    #Fill ENWfDer with the derivative of the wavefunction w.r.t. b and W
    k = 1
    for h=1:alpha
        r_ = (h-1)*nspins; @views t_ = tanh.(Lt[(r_+1):(h*nspins)])
        @views    derivative[h] = sum(t_)
        for i=1:nspins
                @views    lt = state[FilterInd[:,i+r_]]'*t_
                    derivative[alpha + k] = lt
                    k += 1
        end
    end
end
